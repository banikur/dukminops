                                    ...
                                    <div role="content">
                                        <div class="widget-body">
                                            ...
                                            <table id="dt_basic_1" class="table table-hover table-bordered table-striped table-responsive">
                                                ...
                                                <tbody>
                                                    <?php $no=1 ?>
                                                    @foreach($master_provinsi as $mp)
                                                    <tr>
                                                        <td><center>{{ $no++ }}</center></td>
                                                        <td><center>{{ $mp->nama_prov }}</center></td>
                                                        <td><center>
                                                            ...
                                                            <button type="submit" class="btn btn-sm btn-danger" onclick="hapus('{{$mp->id}}')"><i class="fa fa-trash"></i>&nbsp;Delete</button>
                                                        </center></td>
                                                    </tr>
                                                    @endforeach
                                                </tbody>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="exampleModalLabel">Tambah Master Provinsi</h4>
            </div>
            <form id="formTambah" action="{{ url('master/provinsi-dashboard/tambah') }}" method="POST">
            @csrf
  ...
  <div class="modal fade" id="edit-master" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="formEdit" action="{{url('master/provinsi-dashboard/edit')}}" method="post">
            @csrf
                <div class="modal-body">
                    <input type="hidden" id="id_edit" name="id_edit">
                    <div class="form-group">
                        <label for="provinsi_edit" class="col-form-label">Nama Provinsi</label>
...
<script>
    ...
    function EditMaster(obj){
        ...
        $('#id_edit').val(data.id);
        $('#provinsi_edit').val(data.nama_prov);
    ...
    function hapus(id)
        {
            Swal.fire({
              ...
              confirmButtonText: "Hapus",
                ...
                if(result.value){
                    window.location.href = "{{ URL::to('master/provinsi-dashboard/hapus/')}}"+'/'+id;
                }else{
                    Swal.fire({
                        type: 'error',
                        text: "Batal Hapus",
